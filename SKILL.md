...
1. **Usage API** — `GET https://api.anthropic.com/api/oauth/usage` with headers `Authorization: Bearer <accessToken>` and `anthropic-beta: oauth-2025-04-20`. The token lives in Keychain generic password, service `"Claude Code-credentials"`, JSON key `claudeAiOauth.accessToken`.
   ...
   - Tokens expire in ~8h. Refresh: `POST https://console.anthropic.com/v1/oauth/token` with JSON `{grant_type: "refresh_token", refresh_token, client_id: "9d1c250a-e61b-44d9-88ed-5944d1962f5e"}` (Claude Code's public client id). Persist the rotated tokens back to the same Keychain item, preserving all sibling JSON keys, so the CLI stays signed in too.
...
- **Panel behavior**: borderless `NSPanel` with `.nonactivatingPanel`, `isFloatingPanel`, `level = .floating`, `collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary]`, `isMovableByWindowBackground`, `canBecomeKey = false`. Add `.resizable` to the style mask for edge-drag stretching.
