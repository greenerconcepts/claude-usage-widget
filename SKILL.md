...
3. For the **per-model gauge (e.g. Fable) and exact reset times**, a Claude Code CLI sign-in must exist in the Keychain. Check with `claude auth status` (NEVER read the keychain item from a shell — each `security find-generic-password` call from a new process spawns a password prompt for the user). If not logged in: have the user run `claude auth login` (install via `npm i -g @anthropic-ai/claude-code` if needed), choose "Claude account with subscription", authorize in the browser. Verify the key exists afterward — a user can *think* they signed in when the browser step never completed.
...
- **Sign with a stable identity, never ad-hoc.** Ad-hoc signatures change on every rebuild, so macOS treats each build as a new app and re-asks every permission (Keychain Always Allow, Accessibility…). `build.sh` signs with the first codesigning identity in the keychain; create one once with openssl (self-signed, codeSigning EKU, PKCS12 exported with `-certpbe PBE-SHA1-3DES -keypbe PBE-SHA1-3DES -macalg sha1` for `security import`, then `security add-trusted-cert -p codeSign`). Grants then survive rebuilds.
